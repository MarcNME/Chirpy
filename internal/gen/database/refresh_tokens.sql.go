...
func (q *Queries) NewRefreshToken(ctx context.Context, token string, userID uuid.UUID, expiresAt time.Time) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, newRefreshToken, token, userID, expiresAt)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.CreatedAt,
